...
test('home sync row mirrors card primary and secondary alignment with responsive wrapping', () => {
  ...
  assert.match(toolbarSyncSource, /安装用户脚本/);
  ...
  assert.match(style, /\.toolbar-sync-primary \.btn\.primary[\s\S]*?min-height:\s*34px[\s\S]*?padding:\s*0 11px[\s\S]*?border-radius:\s*9px[\s\S]*?font-size:\s*\.78rem[\s\S]*?font-weight:\s*760/s);
  assert.match(style, /\.toolbar-sync-secondary \.bookmarklet\.secondary[\s\S]*?min-height:\s*34px[\s\S]*?padding:\s*0 11px[\s\S]*?border-radius:\s*9px[\s\S]*?font-size:\s*\.78rem[\s\S]*?font-weight:\s*760/s);
  assert.match(style, /\.toolbar-sync-primary \.btn\.primary:disabled\s*\{[^}]*opacity:\s*1/s);
  assert.match(style, /\.bookmarklet\.secondary\s*\{[^}]*color:\s*var\(--text\)[^}]*background:\s*transparent[^}]*border-color:\s*var\(--control-border\)/s);
  ...
  assert.match(style, /\.toolbar-sync-toggle-copy strong\s*\{[^}]*font-size:\s*\.76rem/s);
  assert.match(style, /\.toolbar-sync-toggle-copy small\s*\{[^}]*font-size:\s*\.66rem/s);
